...
class array {
 public:
  ...
  using const_iterator = const T *; // определяет константный тип для итерации по контейнеру
  ...
  array &operator=(array &&other) noexcept;  // Оператор присваивания с перемещением


  reference at(size_type pos);  // доступ к элементу по индексу с проверкой
  const_reference at(
      size_type pos) const;  // константный доступ к элементу по индексу с проверкой
  reference operator[](size_type pos);  // оператор доступа по индексу с проверкой
  const_reference operator[](
      size_type pos) const;  // константный оператор доступа по индексу с проверкой
